...
import SubPrazoProtocolo from "./SubPrazoProtocolo";
import SubPrazoProjeto from "./SubPrazoProjeto";
import Typography from "@mui/material/Typography";

export default function DisplayPrazos() {
  const lista = [
    ...
    <>
      ...
      >
        ...
        <Typography fontWeight="bold" component="h1" variant="body2">
          Aqui estão os próximos projetos e protocolos a serem entregues
        </Typography>
        <Divider>
        ...
        <Box sx={{ maxHeight: "400px", overflowY: "auto" }}>
          {lista.map((protocolo) => (
            <SubPrazoProtocolo key={protocolo.id} protocolo={protocolo} />
          ))}
        </Box>
        <Divider>
          <Chip label="Projetos" />
        ...
        <Box sx={{ maxHeight: "400px", overflowY: "auto" }}>
          {lista.map((projeto) => (
            <SubPrazoProjeto key={projeto.id} projeto={projeto} />
          ))}
        </Box>
      </Card>
    </>
